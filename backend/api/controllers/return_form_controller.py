...
def get_return_forms(self):
    ...
    serializer = GetReturnFormSerializer(rental_forms, many=True)
    return Response(serializer.data, status=status.HTTP_200_OK)

@api_view(['POST'])
def add_return_form(request):
    serializer = SendReturnFormSerializer(data=request.data)
...
def edit_return_form(request, pk):
    try:
        ...
        if rental_form:
            serializer = SendReturnFormSerializer(rental_form)
            return Response(serializer.data, status=status.HTTP_200_OK)
...
def get_return_form_detail(request, pk):
    ...
    if rental_form:
        serializer = GetReturnFormSerializer(rental_form)
        return Response(serializer.data, status=status.HTTP_200_OK)
    return Response({"error": "RentalForm not found"}, status=status.HTTP_404_NOT_FOUND)
